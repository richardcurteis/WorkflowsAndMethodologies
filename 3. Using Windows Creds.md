...
> Working from a Windows attack box utilizing the Windows command line and PowerShell to execute commands via Windows Management Instrumentation (WMI) and Windows Remote Management (WinRM).

### WMI ###

> Windows Management Instrumentation (WMI) is the infrastructure for management data and operations on Windows-based operating systems.

> Allows remote querying and administration of Windows devices. Like SNMP for Windows infrastructure. 
...
> See Nishang and Powersploit for weaponised examples
...
```PS C:\tools\SysinternalsSuite> .\PsExec.exe \\$pcName -u $domain\$user -p nastyCutt3r -h -d powershell.exe "enable-psre  
moting -force"```
